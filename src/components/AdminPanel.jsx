...
import { X, Users, BarChart2, Clock, ShieldOff, ShieldCheck, AlertTriangle, Crown, MessageSquare, CheckCircle2, Building2, ShieldAlert, Plus, ChevronDown, Activity, Trash2, Truck, Edit2, RefreshCw, UserCheck, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[600] flex items-center justify-center p-2">
      <div className="w-full bg-[#0a100c] border border-[#3b82f6]/30 rounded-2xl shadow-[0_0_60px_rgba(59,130,246,0.2)] flex flex-col" style={{height:'calc(100vh - 16px)'}}>
        ...
        {activeTab === 'learn' && (
          <AdminLearnTab
            learnLoading={learnLoading} learnStats={learnStats} learnSug={learnSug} learnCand={learnCand}
            LEARN_SUG_DEFS={LEARN_SUG_DEFS}
            fetchLearn={fetchLearn} approveSug={approveSug} rejectSug={rejectSug} dismissCand={dismissCand}
          />
        )}
        ...
        {activeTab === 'ops' && (
          <AdminOpsTab
            ...
            auditLogs={auditLogs}
            churnRisk={churnRisk}
            cityUsageStats={cityUsageStats}
            companyMigStatus={companyMigStatus}
            errorLoading={errorLoading}
            errorLogs={errorLogs}
            fetchErrorLogs={fetchErrorLogs}
            fetchUsageEvents={fetchUsageEvents}
            migrationStatus={migrationStatus}
            nearLimit={nearLimit}
            noteCleanStatus={noteCleanStatus}
            nowSec={nowSec}
            promoteMigStatus={promoteMigStatus}
            runCompanyUnifyMigration={runCompanyUnifyMigration}
            runNoteCleanMigration={runNoteCleanMigration}
            runOfficeMigration={runOfficeMigration}
            runPromotePersonalCompanies={runPromotePersonalCompanies}
