      ...
      <div className="bg-[#0a100c] border border-[#22c55e]/30 rounded-2xl w-full max-w-md shadow-[0_0_40px_rgba(34,197,94,0.15)] flex flex-col overflow-hidden">
        ...
        <div className="p-5 border-b border-[#1e2d22] flex justify-between items-center">
          <h2 className="text-lg font-black text-white flex items-center gap-2">
            <Database size={18} className="text-[#22c55e]" /> 기본명단 DB에서 이식하기
          </h2>
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
              ...
              {IMPORT_FIELDS.map(f => {
                ...
                return (
                  <button key={f.key} type="button" onClick={() => toggleField(f.key)}
                    className={`flex items-center gap-2 p-3 rounded-xl border text-left transition-all ${checked ? 'bg-[#22c55e]/15 border-[#22c55e]/50 text-[#22c55e]' : 'bg-black/30 border-gray-800 text-gray-500 hover:border-gray-600'}`}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <select value={selectedSido} onChange={e => { setSelectedSido(e.target.value); setSelectedSigungu(''); }}
                className="w-full bg-black/60 border border-[#2d4a35] rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#22c55e] transition-colors">
            ...
            {selectedCity && <p className="text-xs text-[#22c55e] mt-1.5 font-bold">✓ {selectedCity}</p>}
          </div>
          ...
          {fetchedCount !== null && (
            <div className="p-3 bg-[#22c55e]/10 border border-[#22c55e]/30 rounded-xl text-[#22c55e] text-sm font-bold text-center">
