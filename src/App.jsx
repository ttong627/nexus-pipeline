...
⚠️ 동명이인 의심 ${conflictSkipped}건은 건드리지 않았습니다 (기존 값 보존 · 담당자 확인 필요)` : ''}`);
  ...
  // ★인라인 화살표는 렌더마다 새 함수라 자식 memo 를 깬다 — 자주 쓰는 것만 고정한다.
  //   ⚠️핸들러 30여 개를 한꺼번에 useCallback 으로 감싸지는 않는다(2026-08-24 판단):
  //     이 파일엔 이미 억제된 deps 경고가 여럿이라, 의존성을 잘못 적으면 **옛 값을 붙든 채 도는 stale closure** 가
  //     조용히 생긴다. 자동 테스트가 못 잡는 종류이고, 이득(그리드 재렌더 몇 ms)보다 위험이 크다.
  const closePurifyResult = useCallback(() => setPurifyResult(null), []);
