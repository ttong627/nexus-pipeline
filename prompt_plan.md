...
| 4-6 UtilsModal 나머지 9탭 | ⏳ **남음** | 같은 방식 반복(−1,500 예상). **형이 화면 확인한 뒤** 이어가는 것을 권함 |

**왜 4-6 을 여기서 멈추나**: 이 화면들을 지키는 자동 테스트가 없다(스모크는 로그인을 안 해 도달조차 못 한다).
지금까지 8단계를 한 번에 배포하면 어느 단계가 깼는지 찾기 어렵다 — `docs/MANUAL_CHECKLIST.md` 로 한 번 확인한 뒤 나머지를 이어가는 편이 안전하다.
