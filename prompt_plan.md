...
## 📊 최종 지표 (세션 종료 시점)
표기 갈림 **73그룹/318건 → 66그룹/231건**(87건·27% 개선).
소스: `paren-repair`74 · `paren-junk-cleanup`73 · `nested-paren-repair`45 · `building-name-unify`30.
남은 231건은 **전부 건물명 별칭/오타**(DB에 건물명 없어 정답 확정 불가):
`시그니처오피스텔↔시그니처아파트` · `태지빌라↔태지아트빌라` · `유희주택↔숭의유희주택` ·
**`화인빌↔파인빌`(오타)** · `가든시티2차↔빈값` · `박석로25번길 32↔32-5`(부번 차이).

## ▶ 다음 후보 (형 승인 대기)
1. `node scripts/unify-building-name.mjs --allow-majority` — 다수결 통일. **근거가 약함**(DB 정본 아님).
   70%↑ 압도적 다수만 채택하도록 이미 제한됨. dry-run으로 대상 확인 후 판단.
2. **P2** `buildingMgtNo` 미보유 41.9% 보강 조회 → 동일성 판정 키 강화.
