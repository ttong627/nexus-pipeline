...
| 3-5 저줌 하이브리드 | ✅ (2026-08-24) | **컬링은 확대했을 때만 듣는다** — 축소하면 전건이 화면 안이라 라벨이 그대로 그려지던 구간을 덮었다. `buildPinInnerHtml` 에 모드 하나만 붙여 **템플릿은 여전히 한 벌**(불변식 7). 라벨 자리에 같은 높이 빈 칸을 남겨 **핀이 움직이지 않는다**(yAnchor 는 높이 비율이라 그냥 빼면 아래로 밀린다). 요소 8→6 · 라벨 텍스트 레이아웃 0. 덤으로 3-3 추출 때 남은 **죽은 ×N 사본** 제거 |
