...
> 2026-07-30 갱신. **P0 45건 · P1 30건 · P1-b 73건 · 다수결 48건 반영 완료** · **주소DB 장애 해결**.
> **P2 완료(37,045건·mgtNo 100%)** · 정화 누적 **324건** · 표기갈림 607→**276건**.
...
## ✅ P2 건물관리번호 보강 — 완료·반영(37,045건, 커밋 `e5e38f1`)
`scripts/backfill-building-mgtno.mjs --write`. **주소 문자열 무변경**, 식별자 필드만 추가.
- **buildingMgtNo 58.1% → 100%** · `apartmentGroupKey` 32.6% → **74.5%**
- 고유 도로명주소 18,927개만 조회(중복 18,118건 절약) · **동시성 3**(장애 재발 방지) · 매칭 100%
- ⚠️ 표기 갈림 수치가 133건 → 607건으로 '증가'한 것은 **악화가 아니라 측정 범위 확대**다.
  이전엔 buildingMgtNo 보유 51,381건(21,949그룹)만 비교 대상이었고, 지금은 88,426건(37,257그룹)
  전부가 그룹핑돼 **그동안 안 보이던 갈림이 드러난 것**.

## ✅ P1 재실행 — 완료·반영(15건, DB 정본만)
형 선택 = **가장 보수적(DB 정본만)**. 5그룹/15건 반영 + `building_alias` 1개 학습. 재실행 대상 0건.
- `--allow-majority`(79그룹/128건)는 **미채택** — 유일한 이름 교체 `백산다세대주택`×3 vs `벽산빌라`×1에서
  **`백산`이 `벽산`(실존 건설사)의 오타일 가능성**이 있어 형이 보수적 선택.

## 📊 세션 최종 지표
- **식별자**: `standardRoadAddress` 100% · **`buildingMgtNo` 58.1% → 100%** · `apartmentGroupKey` 32.6% → **74.5%**
- **데이터 정화 누적**: P0 45 + P1 30 + P1-b 73 + 다수결 48 + P1재실행 15 = **211건** (+ P2 식별자 37,045건)
- **표기 갈림**: 좁은 기준(구 21,949그룹)에서 318→133건(58%↓). 넓은 기준(현 37,257그룹)에서 568건 남음.
  ⚠️ 두 숫자는 **모집단이 다르다** — P2로 전 레코드가 그룹핑되며 그동안 안 보이던 갈림이 드러난 것.

## ✅ 다수결 통일 2차 — 완료·반영(113건)
형이 `백산다세대주택` 채택 확정 후 `--allow-majority --write --learn` 실행. 재실행 대상 0건(보류 28그룹만).
표기 갈림 **607건 → 276건**(331건 개선).

## 📊 세션 최종 (2026-07-30)
- **식별자**: `standardRoadAddress` 100% · **`buildingMgtNo` 58.1%→100%** · `apartmentGroupKey` 32.6%→**74.5%**
- **데이터 정화 누적 324건**: P0 45 + P1 30 + P1-b 73 + 다수결 48 + P1재실행 15 + 다수결2차 113
  (+ P2 식별자 보강 37,045건, 주소 무변경)
- **표기 갈림**: P2 이후 넓은 기준(37,257그룹)에서 607 → **276건**

## ✅ #8 PGPOOL_MAX 상향 — 완료(리비전 `00040-pvr`)
`PGPOOL_MAX` 미설정(=8) → **15**. Cloud SQL `db-custom-1-3840`·Cloud Run maxScale 10 →
최악 10×15=150으로 여유. **검증: 동시 12요청 12/12 성공(최대 526ms)** — 이전엔 8 초과분이 timeout.

## ✅ #9 주소품질 정기 모니터링 — 완료
`scripts/monitor-address-quality.mjs`(읽기전용·PII 미조회). 측정 4종(괄호붕괴·괄호잡값·표기갈림·
식별자 보유율)을 이전 스냅샷과 비교해 **악화 시에만** 텔레그램 알림(조용한 운영).
- 작업스케줄러 **`NexusAddressQuality`** 매주 월 09:00 · VBS 숨김 실행 · 로그 `scripts/monitor.log`
- **기준선**: 괄호붕괴 0 · 괄호잡값 0 · 표기갈림 85그룹 · mgtNo 100% · 정본 100%
...
1. **동률·근소차 28그룹** — `남경오피스텔`↔`성진남경오피스텔` · `시그니처오피스텔`↔`시그니처아파트`
   같은 별칭. 어느 쪽이 정답인지 데이터로 확정 불가(DB에 건물명 없음).
2. **괄호에 층 정보** — `(서초동, 지하층)` · `(서초동, 1층)`. 건물명이 아니라 상세주소 후보지만
   P1-b 잡값 판정에서 `층` 키워드는 이관 대상인데도 남은 것 = **비법정동 값이 하나뿐이라 미개입**(오판 방지 규칙).
   → 이 규칙을 완화하면 정상 건물명까지 훼손될 수 있어 **의도적으로 남긴 것**.
3. **도로명 부번 차이** — `박석로25번길 32` vs `32-5`. 건물명 통일로는 해결 불가(도로명 자체가 다름).
