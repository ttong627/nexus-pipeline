...
> 표기 갈림 **318건 → 133건(58% 개선)**. 다음 = **P2 buildingMgtNo 보강**(형 승인 대기).
...
## ▶ 다음 (형 승인 대기) — P1 재실행
새로 그룹핑된 대상에 건물명 통일 재적용:
- DB 정본만: 5그룹/**15건**
- `--allow-majority`: 79그룹/**128건**(127건이 빈 건물명 채우기)
- ⚠️ 유일한 이름 교체 = `백산다세대주택`×3 vs `벽산빌라`×1 → 다수결은 `백산다세대주택`이지만
  **`백산`이 `벽산`(실존 건설사)의 오타일 가능성**이 있어 형 판단 필요. 밀린 이름은 특이사항 보존됨.
