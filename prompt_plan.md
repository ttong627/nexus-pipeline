...
> 2026-07-30 갱신. **P0 완료·배포·백필 반영**(커밋 `d971b97`). 다음 = **P1 건물명 통일**(형 승인 대기).

## ▶ 다음 작업 (형 승인 대기) — P1 건물명 통일
P0 백필 후 재측정 결과 **표기 갈림 73그룹/318건이 그대로 남았다.** P0가 고친 45건(괄호 구조 붕괴)과
표기 변이 그룹은 겹치지 않았다. 변이의 실제 원인은 **괄호 내용 오염**이다:
- 비건물명 값이 건물명 자리에: `(장안동, 5층 식당보관, E (장안동))` · `(신설동, 8652 (신설동))` — 괄호 짝은 맞아 P0 판정에 미검출
- 법정동 유/무 차이: `(장안동, 가든시티2차)` 11건 vs `(장안동)` 1건
- 건물명 별칭·오타: 화인빌↔파인빌 · 유희주택↔숭의유희주택 · 태지빌라↔태지아트빌라 · 시그니처오피스텔↔시그니처아파트 · 오스카빌↔꿈꾸는오스카빌
**P1 설계**: 같은 `buildingMgtNo` 그룹의 건물명을 **DB 정본(`building_name`)으로 통일**, DB에 없으면 그룹 내 최다 표기로 통일 + 차이를 `building_alias` 학습사전에 축적(#5-D 표기변이 정규화가 이미 소비). 괄호에서 빠지는 비건물명 값(`5층 식당보관`·`8652`)은 **특이사항으로 이관**(삭제 금지). dry-run 선행 필수.
**P2**: `buildingMgtNo` 미보유 41.9% 보강 조회 → 동일성 판정 키 강화.
