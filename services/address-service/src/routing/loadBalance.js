/**
 * 물량배분(기사별 균등 배분) 코어 — 배송 코어 승격 2026-08-04.
 *
 * ★어디서 왔나
 *   `src/workers/routeWorker.js`(Web Worker) 안에만 있던 로직이다. 워커는 `export` 가 0이라
 *   **브라우저 밖에서는 아무도 쓸 수 없었다** — 서버도, 생성된 배송 프로그램도.
 *   그래서 스튜디오에서 '물량배분'을 체크해도 생성물에 아무것도 따라가지 않았다.
 *   여기로 옮겨 `/v1/routing/load-balance` 로 열면 그 통로가 뚫린다.
 *
 * ★로직은 한 줄도 바꾸지 않았다
 *   운영에서 검증된 배분 결과가 바뀌면 **기사 담당 구역이 바뀐다** = 현장 사고다.
 *   이관 전에 6개 전략의 출력을 골든(`scripts/golden/load-balance-golden.json`)에 고정했고,
 *   `scripts/load-balance-parity.test.mjs` 가 이관 후에도 같은 답이 나오는지 검사한다.
 *
 * ★전략 6종
 *   pca(주성분 축 분할) · hilbert(공간채움곡선) · bestOfPcaHilbert(둘 중 편차 적은 쪽) ·
 *   seedVoronoi(기사 핀 기준) · angular(각도 분할) · dongGroup(행정동 보존)
 *   전부 **체감물량 가중**(임대 20포↑ 할인·계단 가중)으로 균등을 맞춘다.
 *
 * ★입력 필드가 nexus 명단 내부 규격이다
