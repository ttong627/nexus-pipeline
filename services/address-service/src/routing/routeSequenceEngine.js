...
/**
 * 아파트 동(棟) 번호 — `101동` 또는 대시 동호(`101-203호`)에서 뽑는다. 없으면 null.
 *
 * ★2차 패턴의 `호` 는 **필수다**(2026-08-11 수정 · 되돌리지 말 것).
 *   예전엔 `호?` 라 optional 이었고, 그래서 `호` 없는 숫자쌍이면 무엇이든 동으로 읽혔다:
 *     · 도로명 부번   `삼작로 376-1, B동 106호` → **376동**(진짜 동은 `B동`)
 *     · 전화번호      `◆2층 010-3947-7678`      → **10동**
 *     · 괄호 안 지번  `(상대원동 1604-1 공동주택)` → **1604동**
 *   더 나쁜 건 도로 번호가 **앞에** 있어 진짜 동호를 가린다는 점이다 —
 *   `중앙로 265-36, 106-1305호 (신부동, 우방아파트)` 가 106동이 아니라 265동이 됐다.
 *   순번 엔진이 이 값으로 묶으므로 **단독주택 여러 채가 "895동" 한 덩어리**가 되고,
 *   아파트 주민은 **없는 동 번호**로 묶여 동 좌표 조회도 함께 빗나갔다.
 *
 *   전 명단 실측(98,020건 · 16개 명단 · `scripts/measure-dong-parse.mjs`):
 *     · `호` 필수화로 달라지는 건 4,359 — 전화 117 + 도로 부번 4,229 + 괄호 지번 11 = **오탐 4,357 제거**
 *     · 그중 **578건은 가려져 있던 진짜 동을 되찾는다**(265동 → 106동)
 *     · **`호` 없는 정상 동호 표기는 0건**. 남은 2건도 지번(`단현동 354-12`)이라 오탐이었다
 *   A-10(주소 정제)도 같은 결론이다 — 대시 동호는 "`호` 표기가 있을 때만" 동호로 본다.
 */
