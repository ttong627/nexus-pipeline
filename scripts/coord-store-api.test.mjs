...
//
//  ⚠️ **대상 모듈(`src/utils/coordStoreApi.js`)은 현재 휴면이다** — 호출부 0건.
//     이 회귀가 초록이어도 *운영이 그 규칙을 지킨다*는 증거가 아니다(설계서 §6).
//     운영 규칙의 잠금은 `scripts/store-coord-pick.test.mjs`(명단 좌표) ·
//     `services/address-service/scripts/delivery-brief.test.mjs`(내비)에 있다.
//     여기는 **되살릴 때를 대비해 규칙 사본을 그대로 유지**하는 용도로 남긴다.
