...
//    node scripts/fill-list-coords.mjs --all --apply                # 전 명단 순차 채움
// ══════════════════════════════════════════════════════════════════
...
const runOne = async (city, month) => {
  ...
  const stat = {
    ...
    wantDong: 0, gotDong: 0, failed: false,
  };
  const missSample = [];
...
if (ALL) {
  const rows = await listMonths();
  console.log(`전 명단 ${rows.length}개 · ${num(rows.reduce((s, r) => s + r.n, 0))}건 — ${APPLY ? '채움' : '실측(조회만)'}`);
  const results = [];
  const allStarted = Date.now();
  for (const [i, r] of rows.entries()) {
    console.log(`\n${'='.repeat(70)}\n[${i + 1}/${rows.length}] ${r.city} ${r.month}`);
    // ★한 명단이 실패해도 다음으로 넘어간다. 15개를 도는 도중 하나가 죽었다고 나머지를
    //   포기하면, 이미 쓴 시간이 통째로 버려진다. 실패는 아래 요약표에 남는다.
    try {
      const s = await runOne(r.city, r.month);
      if (s) results.push(s);
    } catch (error) {
  ...
  if (failed.length) console.log(`\n⛔ 실패 ${failed.length}개 — 재실행하면 이미 채운 건은 캐시로 빨리 지나갑니다.`);
} else {
  await runOne(CITY, MONTH);
}
