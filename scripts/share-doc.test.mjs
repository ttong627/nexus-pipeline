...
test('부모에는 활성 기사 번호만 정규화해 담는다(읽기 권한 판정용)', () => {
  ...
  // ★2026-08-24: 부모 공유 문서에 **전화번호를 넣지 않는다**. 휴대폰 인증이 보류라 쓰이지도 않는데,
  //   부모 문서는 그 공유의 토큰을 가진 기사 전원에게 읽혀 명부 번호가 통째로 새어나간다.
  assert.equal('driverPhones' in meta, false, '부모 공유 문서에 기사 전화번호가 다시 들어갔다 — 토큰 가진 기사 전원에게 읽힌다');
});
