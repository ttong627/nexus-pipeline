...
| A-10 | 동호 형식 정규화 | **숫자 동(대단지 아파트)** → 대시+호수 4자리 우측 공백 패딩: `101동 203호`→`101-  203호`, `1203호`→`101-1203호`. **한글/영문 동(빌라·연립)** → `동` 그대로 유지: `가동 101호`→`가동 101호`, `B동 104호`→`B동 104호` (대단지는 동이 많아 간결한 대시, 빌라는 가독성 위해 동 표기) |
