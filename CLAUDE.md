...
| A-10 | 동호 형식 정규화 | **순수 숫자 동(대단지 아파트)만** → 대시+호수 4자리 우측 공백 패딩: `101동 203호`→`101- 203호`. **한글/영문/대시 동(빌라·연립·`1-1동`)** → `동` 그대로 유지: `가동 101호`→`가동 101호`, `B동 104호`→`B동 104호`, `1-1동 100호`→`1-1동 100호`. ※ 동(棟) 토큰(가동/나동/B동/101동/1-1동)은 `DETAIL_START_RE`/`DETAIL_MARKER_RE`의 `DONG_UNIT_SRC`로 **상세주소로 인식**(건물명 슬롯·괄호로 새지 않음 — 데이터손실 차단). 절대 되돌리지 말 것 |
