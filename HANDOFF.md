...
2. ~~**`parseAptDong` 오탐**~~ ✅ **완료 (2026-08-11)** — 전 명단 **98,020건 실측** 후 `호` 필수화.
   오탐 4,357건 제거(도로 부번 4,229 · 전화번호 117 · 괄호 지번 11) + **578건은 가려져 있던
   진짜 동을 회복**(`중앙로 265-36, 106-1305호 우방아파트` → 265동 ❌ → 106동 ✅).
   `호` 없는 정상 동호 표기는 **0건**이라 퇴행 없음. `RouteMapModal` 복제본도 SSOT import 로 제거.
   규칙 **DS-18** · 근거 §5-3-B · 회귀 `scripts/apt-dong-parse.test.mjs` 19 PASS ·
   실측 도구 `scripts/measure-dong-parse.mjs`(읽기 전용, 재실행 가능).
