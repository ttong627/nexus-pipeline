...
2. **⚠️ 서울 동대문구 `no_anchor` 136건(2개월 합 268건)** — 미조사
   전 명단 채움에서 동대문만 98.3%(다른 곳 99.8~100%). **미보유가 전부 `no_anchor`** 이고
   주소가 **`한천로58길`** 에 몰려 있다. `/v1/address/match` 는 200 을 주는데
   `/v1/coords/resolve` 만 `no_anchor` → **앵커(`road_codes` 조회·`pickRoadCode`) 쪽 문제로
   보이나 단정하지 말 것**. `scripts/probe-address.mjs` 로 실제 후보를 확인하고 판단하라.
   같은 도로에 몰린 것이라 원인 하나로 268건이 풀릴 가능성이 있다.

3. **`parseAptDong` 오탐 24건** — 도로명 부번을 동으로 읽는다(`동서로 895-24 → 895동`).
   **순번이 단독주택 24채를 한 동으로 묶는다.** 순번 엔진 핵심이라 손대지 않았다 — §5-3-A 참조.
   고치려면 전 명단에서 `호` 없는 정상 동호 표기가 있는지 먼저 실측할 것.

3. ~~동 좌표 미보유 규명~~ ✅ **완료(2026-08-11)** — 설계서 §5-3-A 표 참조.
   **남은 미보유의 80%(503/629 가구·103개 단지)는 VWorld 에 동 정보가 아예 없는 것**이라
   코드로 못 고친다. 나머지 126가구는 대부분 `parseAptDong` 오탐(아래 2번).
   → **더 팔 것이 없다.** 오탐만 고치면 이 항목은 끝이다.
