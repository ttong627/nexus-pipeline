...
### 다음 할 일
- **형이 "배포"라고 하면 V7.16 Hosting 배포** → 그 뒤 실기기에서 「번호 변경 → 기사 링크 재입장」 1회 확인.
- `docs/MANUAL_CHECKLIST.md` 형 확인 항목(지도 핀·브러시·자동배분·순번, 좌표매칭, 기사 비밀번호 입장, 도구 탭, 파일합치기).
- 보류(사유 명시): Phase 3-5 저줌 하이브리드 핀(SSOT 중복 위험) · ResultGrid 일괄 `useCallback`(stale closure 위험) ·
  Cloud Run 콜드스타트 20~23초(min-instances = 비용 결정) · 안양 기본명단 3건(형 판단).
